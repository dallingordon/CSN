...
class WaveformDatasetPreload(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        wav_data = self.wav_data_list[file_idx][:, local_idx]  # Slice based on channel and index
                ...
                # **New code to retrieve previous wav_data based on prev_pred**
        start_idx = local_idx - self.prev_pred  # The negative means i need to pre pad the prev predictions
        
        end_idx = local_idx  # Current index (exclusive)
        if start_idx < 0:
           
            
            prev_wav_data = torch.stack(
            [self.wav_data_list[file_idx][:, i] if i >= 0 else torch.zeros(1)
             for i in range(start_idx, end_idx)],
            dim=1
            ).squeeze()
        else:
            # Slice to get previous wav_data, pad with zeros if necessary
            prev_wav_data = torch.stack(
                [self.wav_data_list[file_idx][:, i] for i in range(start_idx, end_idx)],
                dim=1
            ).squeeze()
        ...
        return wav_data, t_step, target, padded_file_name_integers, seq_t_adjusted,prev_wav_data